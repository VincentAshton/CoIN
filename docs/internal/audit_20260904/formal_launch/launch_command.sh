#!/bin/bash
# CoIN+Replay ratio=0.1 正式 sweep launch wrapper（2026-09-04，用户批准）
# 用途：source env.sh → 锁定 conda env → 唯一命令 run_sweep.sh 0.1 → 记录退出码与状态
set -uo pipefail
...
# 禁止设置 ALLOW_SINGLE_STEP_REPLAY（方案 D 下 0.1 不需要）

LROOT=/root/data/coin/logs/formal/ratio_0.1_20260904_032926
LOG="$LROOT/sweep.log"
{
  echo "===== launch $(date '+%F %T') ====="
  echo "HEAD=$(git rev-parse HEAD)"
  echo "cmd: bash scripts/CoIN_Replay/run_sweep.sh 0.1"
  echo "REPLAY_ACCUM=$REPLAY_ACCUM ENFORCE_MIN_STEPS=$ENFORCE_MIN_STEPS GPUS=$GPUS"
} >> "$LOG"

bash scripts/CoIN_Replay/run_sweep.sh 0.1 >> "$LOG" 2>&1
rc=$?
