#!/bin/bash
# ScienceQA 评估（Truth Alignment）。本 fork 改动：
#   - chunk 后台 PID 逐个收集退出码，任一失败即整次失败（原版 wait 只看最后一个）
#   - 去掉 create_prompt（仅 Reasoning Capability 需要；TA 流程退出码诚实化）
#   - EVAL_DRY_RUN=1 零 GPU 演练（目录契约测试）；EVAL_FAULT_INJECT=1 注入 chunk 死亡
# 调用: [EVAL_DRY_RUN=1] [RESULT_DIR=...] bash 1_eval_sqa.sh <STAGE> <MODELPATH>
...
with open(os.path.join(stage, f"{chunks}_0.jsonl"), "w") as f:
    for q in qs:
        f.write(json.dumps({"question_id": q["question_id"], "text": "dummy"}) + "\n")
# 只合并 chunk 文件（形如 <chunks>_<idx>.jsonl），绝不混入其他 .jsonl 产物
merged = []
...
with open(os.path.join(stage, "merge.jsonl"), "w", encoding="utf-8") as f:
    f.write("\n".join(merged) + "\n")
with open(os.path.join(stage, "output_result.jsonl"), "w", encoding="utf-8") as f:
    json.dump({"acc": 42.0, "correct": 0, "count": len(qs)}, f)
print(f"[dry-run] ScienceQA OK -> {stage}")
PYEOF
    exit $?
fi
