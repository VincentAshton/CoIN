#!/bin/bash
# ImageNet 评估（Truth Alignment，官方 substring 口径，不修改评分规则）。
# 本 fork 改动：修复 ./layground 拼写 bug + RESULT_DIR// 双斜杠；PID 收集 / 去 create_prompt / dry-run。
...
QF="${QUESTION_FILE:-./playground/Instructions_Original/ImageNet/test.json}"
...
print(f"[dry-run] ImageNet OK -> {stage}")
PYEOF
    exit $?
fi
